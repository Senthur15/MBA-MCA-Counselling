...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
 ...
 <body>
    ...
    <div class="jumbotron">
        ...
        $college_id ="SELECT $pref_key from sndprefrence where email='$email'";
        ...
        $open_seats = "SELECT oc from seats where c_id='$co_id'";
        $result1 = $con->query($open_seats);
        $row1 = $result1->fetch_assoc();
        $available_seat=$row1["oc"];
        // echo $available_seat;
        if($available_seat > 0)
        {  $decrmt=$available_seat-1;
            ...
            $updte1 ="UPDATE allotment set c_id='$co_id' WHERE $email='$email'";
            $updteresult =$con->query($updte1);
        ...
        $avi_seats="SELECT $comm from seats where c_id='$co_id'";
        $result1 = $con->query($avi_seats);
        $row1 = $result1->fetch_assoc();
        $available_seat=$row1[$comm];
        // echo $available_seat;
        if($available_seat > 0)
        {  $decrmt=$available_seat-1;
            // echo $decrmt;
        ...
        <tr>COLLEGE IN THE PREFRENCE $i IS ALLOTED<tr><tr>
        <th>DISTRICT</th>
        <th>COLLEGE CODE</th>
        <th>COLLEGE NAME</th>
        <th>COURSE TYPE</th>
